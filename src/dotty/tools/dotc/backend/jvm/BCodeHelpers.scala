...
abstract class BCodeHelpers extends BCodeTypes with BytecodeWriters {
  ...
  def jvmWiseLUB(a: BType, b: BType): BType = {
    ...
    assert(isNonSpecial(a), s"jvmWiseLUB() received a non-plain-class $a")
    assert(isNonSpecial(b), s"jvmWiseLUB() received a non-plain-class $b")
    ...
    assert(isNonSpecial(res), "jvmWiseLUB() returned a non-plain-class.")
    res
  }
   ...
   * can-multi-thread
   */
  def methodSymbols(cd: ast.Trees.TypeDef): List[Symbol] = {
    cd.impl.body collect { case dd: ast.Trees.DefDef => dd.symbol }
  }
  ...
  trait BCForwardersGen extends BCAnnotGen with BCJGenSigGen {
    ...
    def addRemoteExceptionAnnot(isRemoteClass: Boolean, isJMethodPublic: Boolean, meth: Symbol) {
      ...
      if (needsAnnotation) {
        ...
        val arg = ast.Trees.Literal(c) setType c.tpe
  ...
  trait BCClassGen extends BCInnerClassGen {
    ...
    def serialVUID(csym: Symbol): Option[Long] = csym getAnnotation definitions.SerialVersionUIDAttr collect {
      case AnnotationInfo(_, ast.Trees.Literal(const) :: _, _) => const.longValue
    }
