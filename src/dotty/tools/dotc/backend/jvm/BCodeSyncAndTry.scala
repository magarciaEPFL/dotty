...
abstract class BCodeSyncAndTry extends BCodeBodyBuilder {
  ...
  abstract class SyncAndTryBuilder(cunit: CompilationUnit) extends PlainBodyBuilder(cunit) {
     ...
     *
     */
    def genLoadTry(tree: ast.Trees.Try[Type]): BType = {

      val ast.Trees.Try(block, catches, finalizer) = tree
      val kind = tpeTK(tree)

      import ast.Trees.{Typed, Ident, Bind}

      val caseHandlers: List[EHClause] =
        for (ast.Trees.CaseDef(pat, _, caseBody) <- catches) yield {
          pat match {
    ...
    def mayCleanStack(tree: Tree): Boolean = tree exists { t => t.isInstanceOf[ast.Trees.Try] }
