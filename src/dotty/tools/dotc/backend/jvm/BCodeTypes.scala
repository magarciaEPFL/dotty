...
abstract class BCodeTypes extends BCodeIdiomatic {
  ...
  case class Tracked(c: BType, flags: Int, sc: Tracked, ifaces: Array[Tracked], innersChain: Array[InnerClassEntry]) {
     ...
     *     (1) scala.Nothing (the test `isNonSpecial(c)` fails for it)
     *     (2) scala.Boolean (it has null superClass and is not an interface)
     */
    assert(isNonSpecial(c) || isCompilingStdLib /*(1)*/, s"non well-formed plain-type: $this")
    assert(
    ...
    assert(ifaces.forall(i => isNonSpecial(i.c) && i.isInterface), s"non well-formed plain-type: $this")
    ...
    def isSubtypeOf(other: BType): Boolean = {
      assert(isNonSpecial(other), "so called special cases have to be handled in BCodeTypes.conforms()")
  ...
  final def exemplar(csym0: Symbol): Tracked = {
    ...
    val key = brefType(csym.javaBinaryName.toTypeName)
    assert(isNonSpecial(key) || isCompilingStdLib, s"Not a class to track: ${csym.fullName}")
  ...
  final def conforms(a: BType, b: BType): Boolean = {
    if (a.isArray) { // may be null
      ...
      else if (b.isArray)       { isNullType(a) } // documentation only, because `if(isNullType(a)` (above) covers this case already.
      else                      { false }
    }
    else {
